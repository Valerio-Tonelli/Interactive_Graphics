...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	for (let i = 0; i < springs.length; i++) {
		...
		let direction = positions[p1].sub(positions[p0]);
		let currentLength = direction.len();
		
		// Skip if particles are at the same position to avoid division by zero
		if (currentLength === 0) continue;
		
		// Spring force magnitude (Hooke's law)
		let displacement = currentLength - spring.rest;
		let springForceMagnitude = stiffness * displacement;
		
		// Unit vector along the spring
		let unitDirection = direction.div(currentLength);
		
		// Spring force vector
		let springForce = unitDirection.mul(springForceMagnitude);
	...
	// Add damping forces (proportional to velocity)
	for (let i = 0; i < positions.length; i++) {
		let dampingForce = velocities[i].mul(-damping);
	...
	for (let i = 0; i < positions.length; i++) {
		// Update velocity first: v_new = v_old + (F/m) * dt
	...
	// Box collision detection and response
	for (let i = 0; i < positions.length; i++) {
		let pos = positions[i];
		let vel = velocities[i];
		
		// Check collision with each wall of the box (extends from -1 to 1 in all dimensions)
